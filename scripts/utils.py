    ...
    try:
        ...
        for eachfile in file_path_list:
            if not os.path.isfile(eachfile):
                raise FileNotFoundError(f"file not found: {eachfile}")
...
def get_flush_db_process_flags(db_path,drfit_db_name,flip=True):
    if flip:
        ...
        process_flags_df.to_sql(name='process_flags', con=cnx, if_exists='replace', index=False) # 'process_flags' is the table name
    else:
        cnx = sqlite3.connect(db_path+drfit_db_name)
